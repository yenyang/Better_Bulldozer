    ...
    internal static class UIFileUtils
    {
        ...
        internal static void ReadUIFiles(View uiView, string directoryName)
        {
            try
            {
                ...
                {
                    BetterBulldozerMod.Instance.Logger.Error(new Exception("unable to locate UI file directory "), directoryPath);
                    return;
                }
            ...
            {
                BetterBulldozerMod.Instance.Logger.Error(e, "exception reading UI files");
            }
        }
        ...
        internal static string ReadHTML(string fileName, string injectionPostfix = "document.body.appendChild(divYYA);")
        {
            try
            {
                ...
                {
                    return "yyBetterBulldozer.div = document.createElement('div'); yyBetterBulldozer.div.innerHTML = \"" + EscapeToJavaScript(html) + "\"; " + injectionPostfix;
                }
            }
            catch (Exception e)
            {
                BetterBulldozerMod.Instance.Logger.Error(e, $" exception reading HTML file {fileName}");
            }
            return null;
        }
        ...
        internal static string ReadHTML(string fileName)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(html))
                {
                    ...
                    return $"yyBetterBulldozer.div = document.createElement('div'); yyBetterBulldozer.div.innerHTML = \"{EscapeToJavaScript(html)}\"; document.body.appendChild(yyBetterBulldozer.div);";
                }
            }
            catch (Exception e)
            {
        ...
        internal static string ReadJS(string fileName)
        {
            ...
            {
                BetterBulldozerMod.Instance.Logger.Error(e, $" exception reading JS file {fileName}");
            }
        ...
        private static string ReadCSS(string fileName)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(css))
                {
                    ...
                    return $"yyBetterBulldozer.style = document.createElement('style'); yyBetterBulldozer.style.type = 'text/css'; yyBetterBulldozer.style.innerHTML = \"{EscapeToJavaScript(css)}\"; document.head.appendChild(yyBetterBulldozer.style);";
                }
            }
            catch (Exception e)
            {
                BetterBulldozerMod.Instance.Logger.Error(e, $" exception reading CSS file {fileName}");
            }
        ...
        private static string ReadUIFile(string fileName)
        {
            ...
            {
                BetterBulldozerMod.Instance.Logger.Error(e, $" exception reading UI file {fileName}");
            }
